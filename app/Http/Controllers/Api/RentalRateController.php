...
class RentalRateController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $stallId = DB::table('stalls')->insertGetId([
            ...
            'monthly_rate' => $validatedData['monthlyRate'] ?? 0.00,
            'created_at' => now(),
            'updated_at' => now(),
        ]);
...
// In RentalRateController.php -> batchUpdate()
    public function batchUpdate(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'stalls.*.monthlyRate' => 'required|numeric',
        ]);
        ...
        DB::transaction(function () use ($validatedData) {
            ...
            foreach ($validatedData['stalls'] as $stallData) {
                ...
                if ($stall) {
                    ...
                    $oldArea = (float) $stall->area;
                    $newArea = isset($stallData['area']) ? (float) $stallData['area'] : $oldArea;
    ...
    public function update(Request $request, $stall)
    {
        $validatedData = $request->validate([
            ...
            'monthlyRate' => 'sometimes|numeric|min:0',
        ]);
